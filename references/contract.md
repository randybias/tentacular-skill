...
| Ingress NetworkPolicy | `triggers[].type` | Webhook: open ingress; else control-plane ingress (namespaceSelector for tentacular-system on TCP 8080 via direct HTTP) |
...
triggers), ingress allows TCP 8080 from the
`10.0.0.0/8` CIDR (control-plane / API server
network). This permits the MCP server to reach
the workflow engine via the Kubernetes API service
proxy for `wf_run` and cron scheduler triggers.
No ephemeral trigger pods are created.
